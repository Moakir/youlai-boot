...
public class SecurityConfig {
    ...
    public AuthenticationManager authenticationManager(
        ...
        // 动态获取所有认证Provider（包括微信认证等）
        try {
            Map<String, AuthenticationProvider> providerBeans = applicationContext.getBeansOfType(AuthenticationProvider.class);
            for (AuthenticationProvider provider : providerBeans.values()) {
                // 避免重复添加已添加的Provider
                if (provider != daoAuthenticationProvider && provider != smsAuthenticationProvider) {
                    providers.add(provider);
                }
            }
        } catch (Exception e) {
            // 忽略获取其他认证Provider时的异常，确保核心认证功能正常
            log.warn("Failed to load additional authentication providers: {}", e.getMessage());
        }
